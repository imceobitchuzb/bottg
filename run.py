...
def run_fastapi():
    ...
    uv_config = uvicorn.Config(fastapi_app, host=HOST, port=PORT, log_level="warning")
...
def start_tunnel():
    """Запуск Cloudflare Tunnel для автоматического получения HTTPS URL для Telegram Mini App"""
    ...
    try:
        logger.info("Starting Cloudflare HTTPS tunnel for Telegram Mini App...")
        ...
        for line in proc.stderr:
            ...
            if m:
                ...
                print("=" * 65)
                print(f"✨ PUBLIC HTTPS MINI APP URL: {tunnel_url}")
                print("=" * 65)
        ...
        logger.warning(f"Tunnel start error: {e}")

def main():
    print("=" * 65)
    print("🚀  XAUUSD AI PRO TRADER 100/100 — TELEGRAM BOT & MINI APP")
    print("=" * 65)
